 ...
 *
 */
class Property_typemodel extends CI_Model {
    ...
    public function get_property_type()
    {
        ...
        $query = $this->db->get_where('property_type', array('status'=>'1'));
        return $query->result();
    }
    ...
    public function get_property_type_save($data){
        $id = $data['id'];
        if($data['status']==''){
            $data['status'] = '0';
        }
        if($data['sequence']==''){
             $data['sequence'] = $this->db->count_all('property_type');
        }
        if($id){
            $this->db->where('id', $id);
            $result = $this->db->update('property_type', array('pname' => $data['pname'],'sequence' => $data['sequence'],'status' => $data['status'],'datetime'=>date('y-m-d H:m:s')));
            $messge = array('message' => 'Property Type Edit SuccessFull','message_type' => 'success');
            $this->session->set_flashdata('item', $messge);
        }else{
            $result = $this->db->insert('property_type', array('pname' => $data['pname'],'sequence' => $data['sequence'],'status' => $data['status'],'datetime'=>date('y-m-d H:m:s')));
            $messge = array('message' => 'Property Type Add SuccessFull','message_type' => 'success');
            $this->session->set_flashdata('item', $messge);
        }
        if($result){
            return true;
    ...
    function get_list() {
        ...
        $aColumns = array('pname','sequence','datetime','status');
        $aResultColumns = array('id','pname','sequence','datetime','status');
        /* Indexed column (used for fast and accurate table cardinality) */
        $sIndexColumn = "id";
        
        $sTable = "property_type";
        $sjoin = "";
	...
	";
        ...
        foreach ($rResult->result_array() as $aRow) {
            ...
            $row['operation'] .= "<a data-toggle='tooltip' data-original-title='Edit' href='property_type/edit_property_type/{$aRow['id']}' class='green'><i class='icon-pencil bigger-130'></i></a>&nbsp;&nbsp;";
